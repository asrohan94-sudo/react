  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const courseData = courseRes.data.courses || courseRes.data.products || [];
  ...
  useEffect(() => {
    if (activeCategory === "All") {
      ...
      const filtered = courses.filter(c => {
        ...
        if (typeof c.category === "object") return c.category.name === activeCategory;
      ...
      {filteredCourses.length === 0 ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {filteredCourses.map(item => (
            <div
              key={item._id}
              onClick={() => navigate(`/product/${item._id}`)} // ✅ Navigate to product route
